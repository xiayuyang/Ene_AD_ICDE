...
class SimulationSynchronization(object):
    ...
    def __init__(self,
        ...
        BridgeHelper.blueprint_library = self.carla.sim_world.get_blueprint_library()
        BridgeHelper.offset = self.sumo.get_net_offset()


        # spawn the actors in carla to sumo
        carla_spawned_actors = self.carla.spawn_vehicle - set(self.sumo2carla_ids.values())
        carla_ego_actor = self.carla.ego_actor_id
        print("carla_ego_actor:   ", carla_ego_actor)
        for carla_actor_id in carla_spawned_actors:
            carla_actor = self.carla.get_actor(carla_actor_id)
            type_id = BridgeHelper.get_sumo_vtype(carla_actor)
            color = carla_actor.attributes.get('color', None) if self.sync_vehicle_color else None
            if type_id is not None:
                # set the ego vehicle in carla to sumo 
                if carla_ego_actor is not None and carla_actor_id == carla_ego_actor:
                    print("set the ego vehicle in carla to sumo:  ", sumo_actor_id)
                    sumo_actor_id = self.sumo.spawn_actor(type_id, color, True)
                    self.sumo.ego_actor_id = sumo_actor_id
                else:
                    sumo_actor_id = self.sumo.spawn_actor(type_id, color, False)
    ...
    def tick(self):
        ...
        # # see the vehicle setting in /Co-Simulation/Sumo/examples/rou/Town04.rou.xml
