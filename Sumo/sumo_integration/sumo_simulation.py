...
class SumoSimulation(object):
    ...
    def spawn_actor(self, type_id, color=None, is_ego=True):
        """
        Spawns a new actor.
        ...
        # if it is the ego vehicle, set its battery consumption and capcity
        if is_ego:
            traci.vehicle.setParameter(actor_id, "device.battery.actualBatteryCapacity", str(800))
    ...
    def tick(self):
        ...
        # Vehicle's electricity consumption in Wh/s during this time step, 
        # to get the value for one step multiply with the step length; error value: -2^30
        e_c = traci.vehicle.getElectricityConsumption(self.ego_actor_id)
        vel = traci.vehicle.getSpeed(self.ego_actor_id)
        pos = traci.vehicle.getPosition(self.ego_actor_id)
        acc = traci.vehicle.getAcceleration(self.ego_actor_id)
        print("sumo_simulation tick self.ego_actor_id, e_c, pos, vel, acc", self.ego_actor_id, e_c, pos, vel, acc)
        return e_c, pos, vel, acc
        # self.e_c_list.append(e_c)
